...
| OpenCV 运行时 | 待不同镜像实测 | 官方 Wiki 有共建案例，不等同于所有镜像能力 |
| 摄像头/V4L2 后端 | 待实板探测 | 不写死 `/dev/video*`、格式、分辨率或后端 |
| UART/GPIO 运行时映射 | 待实板和设备树确认 | 不从物理引脚直接推导 Linux 编号 |
| RKNN/NPU 部署 | 待版本矩阵和实板验证 | 需要 Toolkit、转换工具、runtime、驱动和模型匹配 |
| 目标板 FPS、延迟、温度和稳定性 | 待实测 | 未实测项目不得声称达标 |
