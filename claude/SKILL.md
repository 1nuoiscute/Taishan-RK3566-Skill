...
用户只请求一个低风险、范围明确的 API、命令、语法或单硬件测试时，读取 `templates/quick-check-intake.md`，完成轻量门禁后只输出自包含的探测/验证代码、预期现象、失败排查、证据保存和下一步，不启动完整方案矩阵。回答必须显式包含这五项，不能省略证据保存方式。
...
用户提供已有工程时，先读取 `references/existing-project-integration.md` 和 `templates/existing-project-change.md`，读取相关源码/配置、建立基线并保留已确认接口；材料不足时只索取最小材料，不生成平行工程。答复必须显式列出原行为基线、最小修改面、回滚办法、原有行为回归和新功能最小验证。需要记录“电脑看到、程序输出、板端执行、评分达成”的证据时，使用 `templates/debug-evidence.md`。
